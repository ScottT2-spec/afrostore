...
export default function AdminStoresPage() {
  ...
  const fetchStores = useCallback(async () => {
    ...
    const res = await api.get<{ stores: StoreItem[]; total: number; pages: number }>(`/api/admin/stores?${params}`);
  ...
  const updateStatus = async (storeId: string, newStatus: string) => {
    ...
    const res = await api.patch(`/api/admin/stores/${storeId}`, { status: newStatus });
    if (res.success) {
