...
type Params = { params: Promise<{ storeId: string; saleId: string }> };
...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (!sale || sale.storeId !== storeId) return error("Flash sale not found", 404);
  return success(sale);
}
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  if (!existing || existing.storeId !== storeId) return error("Flash sale not found", 404);
