...
type Params = { params: Promise<{ storeId: string; affiliateId: string }> };

// GET — affiliate detail with referrals & payouts
export async function GET(req: NextRequest, { params }: Params) {
  const { storeId, affiliateId } = await params;
  ...
  if (affiliate.program.storeId !== storeId) return error("Forbidden", 403);
