...
// GET /api/stores/:storeId/referrals/track?ref=CODE — track click
export async function GET(req: NextRequest, { params }: Params) {
  ...
  if (!affiliate || affiliate.program.storeId !== storeId) {
...
// POST /api/stores/:storeId/referrals/track — convert referral (called on order completion)
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (!order || order.storeId !== storeId) return error("Order not found", 404);
