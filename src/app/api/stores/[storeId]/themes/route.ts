...
// POST /api/stores/:storeId/themes — install/activate a theme
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const storeTheme = await prisma.storeTheme.upsert({
      where: { storeId_themeId: { storeId, themeId } },
      create: {
        storeId,
        themeId,
    ...
    if (activate !== false) {
      await prisma.storeTheme.updateMany({
        where: { storeId, themeId: { not: themeId } },
        data: { isActive: false },
      });
