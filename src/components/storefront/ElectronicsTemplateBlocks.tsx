...
export function ElectronicsFooter({
  ...
  const scopedCss = `
    .el-footer a { color: rgba(255,255,255,0.66); text-decoration: none; transition: color 0.2s; }
    .el-footer a:hover { color: #fff; }

    .el-col-brand { flex: 0 1 25%; min-width: 220px; }
    .el-col-posts { flex: 0 1 25%; min-width: 220px; }
    .el-col-links { flex: 0 1 17%; min-width: 140px; }

    .el-col-toggle-head {
      display: flex; justify-content: space-between; align-items: center;
      cursor: pointer; user-select: none; padding: 0;
    }
    .el-col-toggle-head svg {
      width: 12px; height: 12px; fill: rgba(255,255,255,0.66);
      transition: transform 0.3s;
      display: none;
    }
    .el-col-toggle-head.el-open svg { transform: rotate(180deg); }
    ...
    .el-link-list { list-style: none; margin: 0; padding: 0; }
    .el-link-list li { margin-bottom: 10px; }
    .el-link-list li a { font-size: 14px; }
    .el-link-list li a em { font-style: italic; }

    .el-contact-list { list-style: none; margin: 16px 0 0; padding: 0; }
    .el-contact-item { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 10px; }
    .el-contact-icon { width: 14px; height: 14px; flex-shrink: 0; margin-top: 4px; fill: rgba(255,255,255,0.66); }

    .el-post-item { display: flex; gap: 12px; margin-bottom: 15px; }
    .el-post-thumb { width: 75px; height: 65px; border-radius: 0; object-fit: cover; flex-shrink: 0; }
    .el-post-title { font-size: 14px; color: rgba(255,255,255,0.85); font-weight: 400; margin: 0 0 4px; line-height: 1.4; }
    .el-post-title a { color: rgba(255,255,255,0.85); }
    .el-post-title a:hover { color: #fff; }
    .el-post-date { font-size: 12px; color: rgba(255,255,255,0.45); }
    ...
    .el-copyrights small { font-size: 13px; color: rgba(255,255,255,0.5); }
    .el-copyrights small a { color: rgba(255,255,255,0.5); }
    .el-copyrights img { height: 21px; width: auto; }

    @media (max-width: 768px) {
      .el-main-footer { gap: 0 !important; padding: 0 15px !important; }
      .el-col-brand, .el-col-posts, .el-col-links {
        flex: 0 1 100% !important; min-width: 100% !important;
        border-bottom: 1px solid rgba(255,255,255,0.08);
        padding: 20px 0;
      }
      .el-col-brand { border-bottom: 1px solid rgba(255,255,255,0.08); padding-top: 30px; }
      .el-col-toggle-head svg { display: block; }
      .el-col-toggle-content { overflow: hidden; transition: max-height 0.3s ease; }
      .el-col-toggle-content.el-closed { max-height: 0; }
      .el-col-toggle-content.el-open { max-height: 500px; }
      .el-col-title { margin-bottom: 0; }
      .el-col-toggle-head.el-open .el-col-title { margin-bottom: 15px; }
    }

    @media (min-width: 769px) {
      .el-col-toggle-content { max-height: none !important; }
    }

    @media (min-width: 769px) and (max-width: 1024px) {
      .el-col-brand, .el-col-posts { flex: 0 1 calc(50% - 15px) !important; }
      .el-col-links { flex: 0 1 calc(33% - 20px) !important; }
    }
  `;

  const contactIcons = {
    address: (
      <svg viewBox="0 0 477 477" className="el-contact-icon">
    ...
    phone: (
      <svg viewBox="0 0 27 27" className="el-contact-icon">
    ...
    fax: (
      <svg viewBox="0 0 479 479" className="el-contact-icon">
  ...
  const renderLinkColumn = (col: FooterLinkColumn, idx: number) => {
    ...
    return (
      <div key={idx} className="el-col-links">
        <div
          className={`el-col-toggle-head ${isOpen ? "el-open" : ""}`}
          onClick={() => toggleColumn(colIndex)}
        >
          <h4 className="el-col-title">{col.title}</h4>
          {chevronSvg}
        </div>
        <div className={`el-col-toggle-content ${isOpen ? "el-open" : "el-closed"}`}>
          <ul className="el-link-list">
  ...
  return (
    <footer className="el-footer" style={footerStyle}>
      <ScopedStyles id="footer" css={scopedCss} />

      <div className="el-main-footer" style={mainFooterStyle}>
        <div className="el-col-brand">
          {logoUrl && (
          ...
          {contact && (
            <ul className="el-contact-list">
              {contact.address && (
                <li className="el-contact-item">
        ...
        {recentPosts.length > 0 && (
          <div className="el-col-posts">
            <div
              className={`el-col-toggle-head ${openColumns.has(1) ? "el-open" : ""}`}
              onClick={() => toggleColumn(1)}
            >
              <h4 className="el-col-title">RECENT POSTS</h4>
              {chevronSvg}
            </div>
            <div className={`el-col-toggle-content ${openColumns.has(1) ? "el-open" : "el-closed"}`}>
              {recentPosts.map((post, i) => (
                <div key={i} className="el-post-item">
                  {post.thumbnail && (
                    <img
                      src={post.thumbnail}
                      alt={post.title}
                      className="el-post-thumb"
                      loading="lazy"
                    />
                  )}
                  <div>
                    <h5 className="el-post-title">
                      <Link href={resolveStoreLink(post.url, storeCtx?.storeSlug)}>{post.title}</Link>
                    </h5>
                    <span className="el-post-date">{post.date}</span>
                  </div>
                </div>
              ))}
            </div>
