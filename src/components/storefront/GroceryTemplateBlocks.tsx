...
export function GroceryFooter({
  ...
  const scopedCss = `
    .gc-footer a { color: rgba(255,255,255,0.66); text-decoration: none; transition: color 0.2s; }
    .gc-footer a:hover { color: #fff; }

    .gc-col-brand { flex: 0 1 25%; min-width: 220px; }
    .gc-col-posts { flex: 0 1 25%; min-width: 220px; }
    .gc-col-links { flex: 0 1 17%; min-width: 140px; }

    .gc-col-toggle-head {
      display: flex; justify-content: space-between; align-items: center;
      cursor: pointer; user-select: none; padding: 0;
    }
    .gc-col-toggle-head svg {
      width: 12px; height: 12px; fill: rgba(255,255,255,0.66);
      transition: transform 0.3s;
      display: none;
    }
    .gc-col-toggle-head.gc-open svg { transform: rotate(180deg); }
    ...
    .gc-link-list { list-style: none; margin: 0; padding: 0; }
    .gc-link-list li { margin-bottom: 10px; }
    .gc-link-list li a { font-size: 14px; }
    .gc-link-list li a em { font-style: italic; }

    .gc-contact-list { list-style: none; margin: 16px 0 0; padding: 0; }
    .gc-contact-item { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 10px; }
    .gc-contact-icon { width: 14px; height: 14px; flex-shrink: 0; margin-top: 4px; fill: rgba(255,255,255,0.66); }

    .gc-post-item { display: flex; gap: 12px; margin-bottom: 15px; }
    .gc-post-thumb { width: 75px; height: 65px; border-radius: 0; object-fit: cover; flex-shrink: 0; }
    .gc-post-title { font-size: 14px; color: rgba(255,255,255,0.85); font-weight: 400; margin: 0 0 4px; line-height: 1.4; }
    .gc-post-title a { color: rgba(255,255,255,0.85); }
    .gc-post-title a:hover { color: #fff; }
    .gc-post-date { font-size: 12px; color: rgba(255,255,255,0.45); }
    ...
    .gc-copyrights small { font-size: 13px; color: rgba(255,255,255,0.5); }
    .gc-copyrights small a { color: rgba(255,255,255,0.5); }
    .gc-copyrights img { height: 21px; width: auto; }

    @media (max-width: 768px) {
      .gc-main-footer { gap: 0 !important; padding: 0 15px !important; }
      .gc-col-brand, .gc-col-posts, .gc-col-links {
        flex: 0 1 100% !important; min-width: 100% !important;
        border-bottom: 1px solid rgba(255,255,255,0.08);
        padding: 20px 0;
      }
      .gc-col-brand { border-bottom: 1px solid rgba(255,255,255,0.08); padding-top: 30px; }
      .gc-col-toggle-head svg { display: block; }
      .gc-col-toggle-content { overflow: hidden; transition: max-height 0.3s ease; }
      .gc-col-toggle-content.gc-closed { max-height: 0; }
      .gc-col-toggle-content.gc-open { max-height: 500px; }
      .gc-col-title { margin-bottom: 0; }
      .gc-col-toggle-head.gc-open .gc-col-title { margin-bottom: 15px; }
    }

    @media (min-width: 769px) {
      .gc-col-toggle-content { max-height: none !important; }
    }

    @media (min-width: 769px) and (max-width: 1024px) {
      .gc-col-brand, .gc-col-posts { flex: 0 1 calc(50% - 15px) !important; }
      .gc-col-links { flex: 0 1 calc(33% - 20px) !important; }
    }
  `;

  const contactIcons = {
    address: (
      <svg viewBox="0 0 477 477" className="gc-contact-icon">
    ...
    phone: (
      <svg viewBox="0 0 27 27" className="gc-contact-icon">
    ...
    fax: (
      <svg viewBox="0 0 479 479" className="gc-contact-icon">
    ...
    <footer className="gc-footer">
      ...
      <div style={containerStyle}>
        ...
        {recentPosts.length > 0 && (
          <div className="gc-col-posts">
            <div
              className={`gc-col-toggle-head ${openColumns.has(1) ? "gc-open" : ""}`}
              onClick={() => toggleColumn(1)}
            >
              <h4 className="gc-col-title">RECENT POSTS</h4>
              {chevronSvg}
            </div>
            <div className={`gc-col-toggle-content ${openColumns.has(1) ? "gc-open" : "gc-closed"}`}>
              {recentPosts.map((post, i) => (
                <div key={i} className="gc-post-item">
                  {post.thumbnail && (
                    <img
                      src={post.thumbnail}
                      alt={post.title}
                      className="gc-post-thumb"
                      loading="lazy"
                    />
                  )}
                  <div>
                    <h5 className="gc-post-title">
                      <Link href={resolveStoreLink(post.url, storeCtx?.storeSlug)}>{post.title}</Link>
                    </h5>
                    <span className="gc-post-date">{post.date}</span>
                  </div>
                </div>
              ))}
            </div>
