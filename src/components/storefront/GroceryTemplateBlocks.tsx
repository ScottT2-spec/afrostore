...
export interface GroceryFooterLinkItem {
  label: string;
  href: string;
}

export interface GroceryFooterLinkColumn {
  title: string;
  links: GroceryFooterLinkItem[];
}

export interface GroceryFooterProps {
  logo?: string;
  description?: string;
  columns?: GroceryFooterLinkColumn[];
  contactPhone?: string;
  contactEmail?: string;
  contactAddress?: string;
  paymentImage?: string;
...
export function GroceryFooter({
  ...
  description = "The best grocery store with the freshest products delivered to your door.",
  columns,
  contactPhone = "(+035) 527-1710-70",
  contactEmail = "grocery@google.com",
  contactAddress = "Choose an address",
  paymentImage = `${IMG}/2018/08/payment.png`,
  copyright = "WoodMart © 2026 created by Xtemos Studio.",
}: GroceryFooterProps) {
  const storeCtx = useContext(GroceryStoreContext);

  const defaultColumns: GroceryFooterLinkColumn[] = [
    { title: "Shop", links: [
    ...
    { title: "Connect", links: [
      ...
      { label: "Twitter", href: "#" }, { label: "Pinterest", href: "#" },
    ]},
  ];
  ...
  const css = `
    .gc-footer { background: ${TOKENS.footerBg}; padding: 60px 0 30px; }
    .gc-footer-grid { display: grid; grid-template-columns: 1.5fr repeat(${cols.length}, 1fr) 1.2fr; gap: 40px; margin-bottom: 40px; }
    .gc-footer-logo { height: 30px; margin-bottom: 15px; filter: brightness(0) invert(1); }
    .gc-footer-desc { font-family: ${TOKENS.bodyFont}; font-size: 14px; line-height: 24px; color: rgba(255,255,255,0.6); margin: 0 0 20px; }
    .gc-footer-col-title { font-family: ${TOKENS.titleFont}; font-weight: 500; font-size: 16px; color: #fff; margin: 0 0 15px; text-transform: uppercase; }
    .gc-footer-links { list-style: none; padding: 0; margin: 0; }
    .gc-footer-links li { margin-bottom: 8px; }
    .gc-footer-links a { font-family: ${TOKENS.bodyFont}; font-size: 14px; color: rgba(255,255,255,0.6); text-decoration: none; transition: color 0.3s; }
    .gc-footer-links a:hover { color: #fff; }
    .gc-footer-contact-item { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 15px; }
    .gc-footer-contact-icon { width: 20px; height: 20px; flex-shrink: 0; filter: brightness(0) invert(1); opacity: 0.6; }
    .gc-footer-contact-text { font-family: ${TOKENS.bodyFont}; font-size: 14px; color: rgba(255,255,255,0.6); }
    .gc-footer-bottom { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; }
    .gc-footer-copyright { font-family: ${TOKENS.bodyFont}; font-size: 13px; color: rgba(255,255,255,0.5); }
    .gc-footer-payment img { height: 24px; filter: brightness(0) invert(1); }
    @media (max-width: 767px) { .gc-footer-grid { grid-template-columns: 1fr; } }
  `;
      ...
      <div style={containerStyle}>
        <div className="gc-footer-grid">
          ...
          <div>
            ...
            <div className="gc-footer-contact-item">
              <img className="gc-footer-contact-icon" src={`${IMG}/2020/06/envelope.svg`} alt="" />
              <span className="gc-footer-contact-text">{storeCtx?.contactEmail || contactEmail}</span>
            </div>
            <div className="gc-footer-contact-item">
              <img className="gc-footer-contact-icon" src={`${IMG}/2020/06/svg-wood-food-market-pointer.svg`} alt="" />
              <span className="gc-footer-contact-text">{storeCtx?.contactAddress || contactAddress}</span>
            </div>
          </div>
        </div>
        <div className="gc-footer-bottom">
          <span className="gc-footer-copyright">{copyright}</span>
          <div className="gc-footer-payment"><img src={paymentImage} alt="Payment methods" /></div>
        </div>
      </div>
    </footer>
  );
