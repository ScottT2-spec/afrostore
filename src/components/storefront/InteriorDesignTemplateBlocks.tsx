...
export interface InteriorFooterLinkColumn {
  title: string;
...
export interface InteriorFooterProps {
  logo?: string;
  description?: string;
  columns?: InteriorFooterLinkColumn[];
  paymentImage?: string;
  copyright?: string;
...
export function InteriorFooter({
  logo = `${IMG}/2018/03/wood-logo-white.svg`,
  description = "Quality interior design products for your home and office.",
  columns,
  paymentImage = `${IMG}/2018/08/payment.png`,
  copyright = "WoodMart © 2026 created by Xtemos Studio.",
  appStoreImage = `${IMG}/2018/08/app-store-button.png`,
  googlePlayImage = `${IMG}/2018/08/google-play-button.png`,
}: InteriorFooterProps) {
  const storeCtx = useContext(InteriorStoreContext);

  const defaultColumns: InteriorFooterLinkColumn[] = [
    { title: "Shop", links: [
  ...
  const css = `
    .id-footer { background: ${TOKENS.footerBg}; padding: 60px 0 30px; }
    .id-footer-grid { display: grid; grid-template-columns: 1.5fr repeat(${cols.length}, 1fr) 1.2fr; gap: 40px; margin-bottom: 40px; }
    .id-footer-logo { height: 30px; margin-bottom: 15px; }
    .id-footer-desc { font-family: ${TOKENS.bodyFont}; font-size: 14px; line-height: 24px; color: rgba(255,255,255,0.6); margin: 0 0 20px; }
    .id-footer-col-title { font-family: ${TOKENS.titleFont}; font-weight: 600; font-size: 16px; color: #fff; margin: 0 0 15px; text-transform: uppercase; }
    .id-footer-links { list-style: none; padding: 0; margin: 0; }
    .id-footer-links li { margin-bottom: 8px; }
    .id-footer-links a { font-family: ${TOKENS.bodyFont}; font-size: 14px; color: rgba(255,255,255,0.6); text-decoration: none; transition: color 0.3s; }
    .id-footer-links a:hover { color: #fff; }
    .id-footer-app-btn { display: block; margin-bottom: 10px; }
    .id-footer-app-btn img { height: 40px; }
    .id-footer-bottom { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 20px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; }
    .id-footer-copyright { font-family: ${TOKENS.bodyFont}; font-size: 13px; color: rgba(255,255,255,0.5); }
    .id-footer-payment img { height: 24px; filter: brightness(0) invert(1); }
    @media (max-width: 767px) { .id-footer-grid { grid-template-columns: 1fr; } }
  `;
