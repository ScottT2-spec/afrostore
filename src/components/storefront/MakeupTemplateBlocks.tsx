            ...
            <div key={p.id} className="mpg-card">
              <div className="mpg-thumb">
                ...
                <div className="mpg-actions">
                  ...
                  <button className="mpg-action-btn" title="Quick view" aria-label="Quick view">👁</button>
                  <button className="mpg-action-btn" title="Wishlist" aria-label="Wishlist">♡</button>
                </div>
...
/* ═══════════════════════════════════════════════════════════════
   11. MAKEUP FOOTER
   Dark footer with logo, link columns, rating widget (4.5/5 stars),
   social icons, copyright bar.
   ═══════════════════════════════════════════════════════════════ */

export interface MakeupFooterLinkColumn {
  title: string;
  links: Array<{ label: string; url: string }>;
}

export interface MakeupFooterProps {
  logoUrl?: string;
  logoAlt?: string;
  description?: string;
  linkColumns?: MakeupFooterLinkColumn[];
  rating?: number;
  ratingCount?: number;
  socialLinks?: Array<{ platform: string; url: string }>;
...
export function MakeupFooter({
  logoUrl,
  logoAlt = "Store Logo",
  description = "Premium beauty and skincare products curated for your self-care routine.",
  linkColumns = [],
  rating = 4.5,
  ratingCount = 374,
  socialLinks = [],
  ...
  const socialIcons: Record<string, string> = {
    ...
    linkedin: "in", pinterest: "📌", telegram: "✈",
  };

  const scopedCss = `
    .mf-footer { background: ${backgroundColor}; color: rgba(255,255,255,0.65); font-family: ${TOKENS.bodyFont}; font-size: 14px; line-height: 1.7; }
    .mf-footer a { color: rgba(255,255,255,0.65); text-decoration: none; transition: color 0.2s; }
    .mf-footer a:hover { color: #fff; }
    .mf-main {
      max-width: ${TOKENS.containerWidth}; margin: 0 auto; padding: 50px 15px;
      display: flex; flex-wrap: wrap; gap: 30px;
    }
    .mf-col-brand { flex: 0 1 28%; min-width: 220px; }
    .mf-col-links { flex: 0 1 18%; min-width: 140px; }
    .mf-col-rating { flex: 0 1 22%; min-width: 180px; text-align: center; }
    .mf-col-title {
      font-family: ${TOKENS.titleFont}; font-weight: 700; font-size: 15px;
      color: #fff; text-transform: uppercase; margin: 0 0 18px; letter-spacing: 0.3px;
    }
    .mf-link-list { list-style: none; margin: 0; padding: 0; }
    .mf-link-list li { margin-bottom: 8px; }
    .mf-rating-num {
      font-family: ${TOKENS.titleFont}; font-weight: 700; font-size: 24px; color: #fff;
    }
    .mf-rating-highlight { color: ${TOKENS.primaryColor}; }
    .mf-stars { color: ${TOKENS.starColor}; font-size: 18px; letter-spacing: 2px; margin: 8px 0; }
    .mf-rating-text { font-size: 13px; color: rgba(255,255,255,0.5); margin-bottom: 15px; }
    .mf-review-btn {
      display: inline-block; padding: 8px 20px; border: 1px solid rgba(255,255,255,0.3);
      border-radius: 25px; color: #fff; font-size: 13px; font-weight: 600;
      text-decoration: none; transition: all 0.3s;
    }
    .mf-review-btn:hover { border-color: #fff; background: rgba(255,255,255,0.1); }
    .mf-social { display: flex; gap: 8px; justify-content: center; margin-top: 15px; }
    .mf-social-icon {
      width: 35px; height: 35px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.25);
      display: flex; align-items: center; justify-content: center;
      color: rgba(255,255,255,0.7); font-size: 13px; font-weight: 700;
      transition: all 0.2s;
    }
    .mf-social-icon:hover { border-color: #fff; color: #fff; }
    .mf-copyrights {
    ...
    .mf-copyrights small { font-size: 13px; color: rgba(255,255,255,0.45); }
    .mf-copyrights img { height: 21px; width: auto; }
    .mf-col-toggle-head {
      display: flex; justify-content: space-between; align-items: center;
      cursor: pointer; user-select: none;
    }
    .mf-col-toggle-head svg {
      width: 12px; height: 12px; fill: rgba(255,255,255,0.6);
      transition: transform 0.3s; display: none;
    }
    .mf-col-toggle-head.mf-open svg { transform: rotate(180deg); }
    @media (max-width: 768px) {
      .mf-main { gap: 0 !important; padding: 0 15px !important; }
      .mf-col-brand, .mf-col-links, .mf-col-rating {
        flex: 0 1 100% !important; min-width: 100% !important;
        border-bottom: 1px solid rgba(255,255,255,0.08); padding: 20px 0;
      }
      .mf-col-toggle-head svg { display: block; }
      .mf-col-toggle-content { overflow: hidden; transition: max-height 0.3s ease; }
      .mf-col-toggle-content.mf-closed { max-height: 0; }
      .mf-col-toggle-content.mf-open { max-height: 500px; }
      .mf-col-title { margin-bottom: 0; }
      .mf-col-toggle-head.mf-open .mf-col-title { margin-bottom: 15px; }
    }
    @media (min-width: 769px) { .mf-col-toggle-content { max-height: none !important; } }
  `;
