...
export function MakeupFooter({
  ...
  const scopedCss = `
    .mk-footer a { color: rgba(255,255,255,0.66); text-decoration: none; transition: color 0.2s; }
    .mk-footer a:hover { color: #fff; }

    .mk-col-brand { flex: 0 1 25%; min-width: 220px; }
    .mk-col-posts { flex: 0 1 25%; min-width: 220px; }
    .mk-col-links { flex: 0 1 17%; min-width: 140px; }

    .mk-col-toggle-head {
      display: flex; justify-content: space-between; align-items: center;
      cursor: pointer; user-select: none; padding: 0;
    }
    .mk-col-toggle-head svg {
      width: 12px; height: 12px; fill: rgba(255,255,255,0.66);
      transition: transform 0.3s;
      display: none;
    }
    .mk-col-toggle-head.mk-open svg { transform: rotate(180deg); }
    ...
    .mk-link-list { list-style: none; margin: 0; padding: 0; }
    .mk-link-list li { margin-bottom: 10px; }
    .mk-link-list li a { font-size: 14px; }
    .mk-link-list li a em { font-style: italic; }

    .mk-contact-list { list-style: none; margin: 16px 0 0; padding: 0; }
    .mk-contact-item { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 10px; }
    .mk-contact-icon { width: 14px; height: 14px; flex-shrink: 0; margin-top: 4px; fill: rgba(255,255,255,0.66); }

    .mk-post-item { display: flex; gap: 12px; margin-bottom: 15px; }
    .mk-post-thumb { width: 75px; height: 65px; border-radius: 0; object-fit: cover; flex-shrink: 0; }
    .mk-post-title { font-size: 14px; color: rgba(255,255,255,0.85); font-weight: 400; margin: 0 0 4px; line-height: 1.4; }
    .mk-post-title a { color: rgba(255,255,255,0.85); }
    .mk-post-title a:hover { color: #fff; }
    .mk-post-date { font-size: 12px; color: rgba(255,255,255,0.45); }
    ...
    .mk-copyrights small { font-size: 13px; color: rgba(255,255,255,0.5); }
    .mk-copyrights small a { color: rgba(255,255,255,0.5); }
    .mk-copyrights img { height: 21px; width: auto; }

    @media (max-width: 768px) {
      .mk-main-footer { gap: 0 !important; padding: 0 15px !important; }
      .mk-col-brand, .mk-col-posts, .mk-col-links {
        flex: 0 1 100% !important; min-width: 100% !important;
        border-bottom: 1px solid rgba(255,255,255,0.08);
        padding: 20px 0;
      }
      .mk-col-brand { border-bottom: 1px solid rgba(255,255,255,0.08); padding-top: 30px; }
      .mk-col-toggle-head svg { display: block; }
      .mk-col-toggle-content { overflow: hidden; transition: max-height 0.3s ease; }
      .mk-col-toggle-content.mk-closed { max-height: 0; }
      .mk-col-toggle-content.mk-open { max-height: 500px; }
      .mk-col-title { margin-bottom: 0; }
      .mk-col-toggle-head.mk-open .mk-col-title { margin-bottom: 15px; }
    }

    @media (min-width: 769px) {
      .mk-col-toggle-content { max-height: none !important; }
    }

    @media (min-width: 769px) and (max-width: 1024px) {
      .mk-col-brand, .mk-col-posts { flex: 0 1 calc(50% - 15px) !important; }
      .mk-col-links { flex: 0 1 calc(33% - 20px) !important; }
    }
  `;

  const contactIcons = {
    address: (
      <svg viewBox="0 0 477 477" className="mk-contact-icon">
    ...
    phone: (
      <svg viewBox="0 0 27 27" className="mk-contact-icon">
    ...
    fax: (
      <svg viewBox="0 0 479 479" className="mk-contact-icon">
          ...
          <div className="mk-col-posts">
            ...
            <div className={`mk-col-toggle-content ${openColumns.has(1) ? "mk-open" : "mk-closed"}`}>
              {recentPosts.map((post, i) => (
                <div key={i} className="mk-post-item">
                  {post.thumbnail && (
                    <img
                      src={post.thumbnail}
                      alt={post.title}
                      className="mk-post-thumb"
                      loading="lazy"
                    />
                  )}
                  <div>
                    <h5 className="mk-post-title">
                      <Link href={resolveStoreLink(post.url, storeCtx?.storeSlug)}>{post.title}</Link>
                    </h5>
                    <span className="mk-post-date">{post.date}</span>
                  </div>
                </div>
              ))}
            </div>
