...
export function MakeupAboutHero({ subtitle, title, bodyText, images, ctaText, ctaLink }: MakeupAboutHeroProps) {
  const storeCtx = useContext(MakeupStoreContext);
  const fixLink = (link?: string) => resolveStoreLink(link || "#", storeCtx?.storeSlug);
...
export function MakeupTextSection({ sectionTitle, bodyText, backgroundColor = "transparent" }: MakeupTextSectionProps) {
  const scopedCss = `
  ...
  return (
    <div className="mk-text-section" style={{ backgroundColor }}>
...
export function MakeupTeamSection({ sectionTitle, team }: MakeupTeamSectionProps) {
  const scopedCss = `
