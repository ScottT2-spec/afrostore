...
export default function TemplatePreview({ template, previewMode = false }: Props) {
  const sections = (template.package?.pages?.[0]?.blocks || template.themeConfig.sections).map((section) => section.type.replace(/_/g, " "));
          ...
          <aside className="space-y-5">
            {previewMode && (
              <div className="rounded-xl border border-brand-200 bg-brand-50 px-4 py-3 text-sm font-medium text-brand-700">
                Internal preview - this template is rendered from the imported package model.
              </div>
            )}
            <div>
